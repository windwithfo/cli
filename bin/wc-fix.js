...
program.option('-c, --config', 'fix project.config.js')
program.action(async function (args) {
  if (args.config) {
    // let proCfg = proCfg = (require(path.join(process.cwd(), 'project.config.js')))
    try {
      fs.copyFileSync(path.resolve(__dirname, '../lib/fix/project.config.js'), path.join(process.cwd(), 'project.config.js'))
      Log('******************************************************************', 'green')
  ...
  if (args.package) {
    ...
    let temp
    try {
      temp = require(`../lib/fix/${proCfg.view}/${proCfg.build}.package.js`)
    } catch (error) {
